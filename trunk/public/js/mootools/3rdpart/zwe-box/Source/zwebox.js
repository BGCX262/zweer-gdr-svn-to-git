var ZweBox = new Class({
    ...
    _createOverlay: function()
    {
        this.elements.overlay = new Overlay(this.options.inject, {
            ...
            onClick: function(e){
                if(this.image.options.hideOnClick)
                    this.close(e);
            }.bind(this)
        });
    },
    ...
    _addEvents: function()
    {
        ...
        document.addEvent('keydown', function(e){
            if(this.visible)
            {
                if(e.code == 27 && this.image.options.hideOnClick) // esc
                {
                    this.close();
                }
                ...
                if(this.gallery && this.gallery.total > 1)
                {
                    ...
                    {
                        this.elements.buttons.prev.fireEvent('click', e);
                    }

                    if(e.code == 39) // right arrow
                    {
                        this.elements.buttons.next.fireEvent('click', e);
                    }
                }
            }
        }.bind(this));
    ...
    maxiMinimize: function(forceMinimize)
    {
        if(this.maximized || forceMinimize)
        {
            ...
            if(!forceMinimize)
                this._loading();
            var objSize = this._calculate(this.image.width, this.image.height);
            if(this.image.options.force)
                this._resize(this.image.options.width, this.image.options.height);
            else if(this.image.options.autoResize)
            {
                if(this.mode == 'image')
                ...
                this._resize(this.image.width, this.image.height);
        }
        else
        {
            this.maximized = true;
    ...
    _unserialize: function(currentElement)
    {
        ...
        data = href.slice(data.indexOf('?') + 1).split('&');
    ...
    _appendHtml: function(element, mode)
    {
        ...
        this.elements.background.addEvent('complete', function(){
            ...
            if(this.image.options.cufon && Cufon && typeOf(Cufon) != null)
                Cufon.refresh();
        }.bind(this));
    },
    ...
    {
        if(this.image.options.flashVars && this.image.options.flashVars == '')
            this.image.options.flashVars = 'autostart=1&autoplay=1&fullscreenbutton=1';

        var str  = '<object width="' + this.image.options.width + '" height="' + this.image.options.height + '" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">';
            ...
            str += '<param name="flashvars" value="' + this.image.options.flashVars + '" />';
            str += '<param name="width" value="' + this.image.options.width + '" />';
            str += '<param name="height" value="' + this.image.options.height + '" />';
            str += '<embed src="' + href + '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" autostart="true" autoplay="true" flashvars="' + this.image.options.flashVars + '" wmode="transparent" width="' + this.image.options.width + '" height="' + this.image.options.height + '" style="margin:0; padding:0;"></embed>';
            str += '</object>';
        return str;
    },
